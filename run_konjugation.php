...
if(!checkIfValidQuantity($quantity)) {
    header('Location: konjugation.php');
    die();
}
?>
<!DOCTYPE html>
...
<head>
    <meta charset="UTF-8">
    <title>Vokabeltrainer Konjugation</title>
    ...
    <script>
        ...
        function Vokabelpaar(deutsch, fremdsprache, id, zeitform) {
        ...
        function goToStart() {
            if (window.confirm("Wirklich abbrechen?") == true) {
                window.location = "konjugation.php";
            }
        }
        ...
        function sendFeedback(idParam,correctParam) {
            $.ajax({
                type: "POST",
                url: "ajax/sendKonjugationFeedbackToDB.php",
                data: {
                    id : idParam,
            ...
            $.ajax({
                ...
                success: function(answer) {
                    ...
                    for (x in phpresponse) {
                        ...
                        vokabelpaare.push(new Vokabelpaar(phpresponse[x].Deutsch,decodeURIComponent(phpresponse[x].Fremdsprache),phpresponse[x].id, phpresponse[x].Zeitform));
                    }
                },
                error: function(){
