...
<head>
    <meta charset="UTF-8">
    <title>Vokabeltrainer Löschen</title>
    ...
    <script>
        ...
        function deleteVocabulary() {
            ...
            if (window.confirm("Diese Vokabel wirklich löschen?") == true) {
                $.ajax({
                    type: "POST",
                    // hier auch ein synchroner aufruf
                    async: false,
                    url: "ajax/deleteVocabularyForId.php",
                    data: { vocabularyId : val },
                    cache: false,
                    success: function() {
                        alert("Vokabel wurde gelöscht");
                    },
                    error: function(){
                        alert("Fehler, kann die Vokabel grad nicht löschen.");
                    }
                });
                location.reload();
            }
